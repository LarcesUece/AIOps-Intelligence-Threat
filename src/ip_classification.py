...
def setup_logging(log_file=None):
    ...
    """
    if log_file is None:
        log_file = os.path.join(OUTPUT_DIR, "ip_classification.log")
...
def classify_ips_and_generate_plots(output_dir=None):
    """Classify IPs using FeatureNormalizer and generate plots.

    This function uses FeatureNormalizer to process the dataset, classify IPs,
    and generate distribution plots.

    Args:
        output_dir (str): Directory to save the plots. Defaults to OUTPUT_DIR from config.
    ...
    with open(temp_config_path, "w") as f:
        json.dump(IP_CLASSIFICATION_CONFIG, f, indent=2)

    try:
        normalizer = FeatureNormalizer(temp_config_path)
    ...
    scores = normalizer.apply_weights_to_new_dataset(
    ...
    classification = pd.cut(
        ...
        bins=[
            -np.inf,
            CLASSIFICATION_THRESHOLDS["allowlist_max"],
            CLASSIFICATION_THRESHOLDS["suspicious_max"],
            np.inf,
        ],
        labels=CLASSIFICATION_THRESHOLDS["labels"],
    )
    ...
    logger.info("   Generating segmented line plot...")
    plt.figure(figsize=FIGURE_SIZE)

    sorted_scores = np.sort(scores_sum)
    count = np.arange(1, len(sorted_scores) + 1)

    allowlist_mask = sorted_scores < CLASSIFICATION_THRESHOLDS["allowlist_max"]
    suspicious_mask = (sorted_scores >= CLASSIFICATION_THRESHOLDS["allowlist_max"]) & (
        sorted_scores < CLASSIFICATION_THRESHOLDS["suspicious_max"]
    )
    blocklist_mask = sorted_scores >= CLASSIFICATION_THRESHOLDS["suspicious_max"]
    ...
    if np.any(allowlist_mask):
        plt.plot(
            ...
            linewidth=4.0,
            color="#1f77b4",
            label=f"Allowlist (<{CLASSIFICATION_THRESHOLDS['allowlist_max']})",
        )
    ...
    if np.any(suspicious_mask):
        plt.plot(
            ...
            linewidth=4.0,
            color="#808080",
            label=f"Suspicious ({CLASSIFICATION_THRESHOLDS['allowlist_max']}-{CLASSIFICATION_THRESHOLDS['suspicious_max']})",
        )
    ...
    if np.any(blocklist_mask):
        plt.plot(
            ...
            linewidth=4.0,
            color="red",
            label=f"Denylist (>{CLASSIFICATION_THRESHOLDS['suspicious_max']})",
        )
    ...
    line_distribution_path = os.path.join(
        output_dir, "score_distribution_line_segments.png"
    )
    plt.savefig(line_distribution_path, bbox_inches="tight", dpi=300)
    plt.close()
    logger.info(f"   Segmented line plot saved at: {line_distribution_path}")

    logger.info("   Generating colored segments plot...")
    plt.figure(figsize=FIGURE_SIZE)
    ...
    plt.axvspan(
        0.4, max(sorted_scores) + 0.05, alpha=0.2, color="red", label="Denylist (>0.4)"
    )
    ...
    labels = ["Allowlist (<0.2)", "Suspicious (0.2-0.4)", "Denylist (>0.4)"]
    ...
    plt.savefig(line_distribution_path, bbox_inches="tight", dpi=300)
    plt.close()
    logger.info(f"   Colored segments plot saved at: {line_distribution_path}")
