...
from src.feature_weight_calculator import calculate_final_weights
from src.ip_classification import classify_ips_and_generate_plots
...
def setup_logging(log_file=None):
    ...
    """
    if log_file is None:
        log_file = os.path.join(OUTPUT_DIR, "full_pipeline.log")
...
def execute_full_pipeline(config_path=None, output_dir=None):
    """Execute the full analysis pipeline:
    1. Calculate final weights.
    2. Classify IPs and generate plots.

    Args:
        config_path (str): Path to the configuration file. If None, uses config.py. Defaults to None.
        output_dir (str): Directory to save the results. If None, uses OUTPUT_DIR from config. Defaults to None.
    ...
    try:
        result = classify_ips_and_generate_plots(output_dir=output_dir)
        logger.info(
